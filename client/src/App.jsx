...
function App() {
  ...
  return (
    <>
      ...
      { !error &&
        <>
          ...
          <div className="clientApiUsage">
            ...
            <button disabled={!viewerControl} onClick={() => viewerControl.changeToPrevPage()}>Previous Page</button>
            <button disabled={!viewerControl} onClick={() => viewerControl.changeToNextPage()}>Next Page</button>
          </div>
        </>
      }
      { error &&
      ...
      <ol>
      <li>
          <p>The client-side react application asked the example application server to create a viewing session for a specific document:</p>
          <p><img src="/images/viewing-sequence-diagrams/react-sample-diagrams.003.png" width="960" alt="React app asks for new viewing session" /></p>
        </li>
        <li>
          <p>The application server <code>POST</code>ed to PAS (PrizmDoc Application Services) to create a new viewing session (in this sample, the application server is a Node.js app, but your actual application server could be anything, like Java, .NET, etc.):</p>
          <p><img src="/images/viewing-sequence-diagrams/react-sample-diagrams.004.png" width="960" alt="POST /ViewingSession" /></p>
        </li>
        <li>
          <p>The application server received a <code>viewingSessionId</code> of <code style={{ fontWeight: 'bold' }}>"{viewingSessionId}"</code>:</p>
          <p><img src="/images/viewing-sequence-diagrams/react-sample-diagrams.005.png" width="960" alt="Receive viewingSessionId" /></p>
        </li>
        <li>
          <p>The application server sent the <code>viewingSessionId</code> to the client-side react application so that it could create the HTML viewer control:</p>
          <p><img src="/images/viewing-sequence-diagrams/react-sample-diagrams.006.png" width="960" alt="Initialize viewer control with the viewingSessionId" /></p>
        </li>
        <li>
          <p>The application server uploaded the source document to PAS, associating that document with the viewing session:</p>
          <p><img src="/images/viewing-sequence-diagrams/react-sample-diagrams.007.png" width="960" alt="Upload source document" /></p>
        </li>
        <li>
          ...
          <p><img src="/images/viewing-sequence-diagrams/react-sample-diagrams.008.png" width="960" alt="Conversion starts" /></p>
        </li>
        <li>
          ...
          <p><img src="/images/viewing-sequence-diagrams/react-sample-diagrams.009.png" width="960" alt="Viewer requests first page" /></p>
        </li>
        <li>
          ...
          <p><img src="/images/viewing-sequence-diagrams/react-sample-diagrams.010.png" width="960" alt="Viewer receives first page" /></p>
        </li>
        <li>
          <p>As you navigate through the document, the viewer will continue to request page content as needed.</p>
          <p><img src="/images/viewing-sequence-diagrams/react-sample-diagrams.011.png" width="960" alt="Viewer continues making requests" /></p>
        </li>
      </ol>
      ...
      <p>
        ...
        That's why the example application server defines a proxy route to PAS at <code>server/pas-proxy/</code> (see <code>server/app.js</code> and
        <code>server/pas/createProxyRouteToPAS.js</code>).
        If you look at the network traffic in dev tools, you'll see the requests made by the viewer for resources rooted at
        this path.
      </p>
